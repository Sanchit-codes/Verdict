...
|----------|--------|---------|
| `/api/generate` | `POST` | Generate text with Gemini and validate in single request |
...
Generate text with Google Gemini and validate the output in a single request. The response includes both the generated text and validation decision.
...
```json
{
  ...
  "context": "Renewable energy refers to energy from natural sources that are constantly replenished...",
  "policy": "default",
  "domain": "energy",
...
|-------|------|----------|-------------|
| `prompt` | string | ✓ | User query or instruction for text generation |
| `context` | string | ✗ | Reference context for validation (e.g., RAG knowledge base) |
| `policy` | string | ✗ | Validation policy name (default: "default") |
| `domain` | string | ✗ | Domain context (e.g., "medical", "finance", "energy") |
...
```json
{
  "generated_text": "Renewable energy provides numerous benefits including environmental sustainability...",
  "decision": "allow",
  "risk_score": 0.18,
  "confidence": 0.91,
  "evidence": "Output matches context with high cosine similarity (0.89).",
  "latency_ms": {
    "generation_ms": 1200,
    "validation_ms": 52
  },
  ...
  "tier_results": [
    {
      ...
      "score": 0.82,
      "passed": true,
      "evidence": "Entity overlap: 0.9, context coverage: 0.85",
      "latency_ms": 2.1
    },
    {
...
|-------|------|-------------|
| `generated_text` | string | The text generated by Gemini (same input to validation) |
| `decision` | string | Validation decision: `allow`, `block`, `regenerate`, `abstain` |
| `risk_score` | number | Risk score 0.0-1.0 (0=safe, 1=dangerous) |
| `confidence` | number | Confidence in decision 0.0-1.0 |
| `evidence` | string | Human-readable explanation for decision |
| `latency_ms` | object | Breakdown: `generation_ms` (Gemini), `validation_ms` (Guard) |
| `policy_name` | string | Name of policy used for validation |
| `tier_results` | array | Per-validator results from 3-tier cascade |
...
```json
{
  ...
  "details": {
    ...
    "reason": "API quota exceeded or service error"
  }
}
```
...
if (result.decision === "allow") {
  ...
  console.log("Blocked:", result.evidence);
}
```
