      ...
      <div className="space-y-1 text-xs">
        ...
        {validation.latency && (
          <p className="text-opacity-70 mt-1">
            ⏱️ Generated {(validation.latency.generation_ms / 1000).toFixed(1)}s | Validated{' '}
            {validation.latency.validation_ms}ms
          </p>
        )}
        {!validation.latency && validation.latency_ms && (
          <p className="text-opacity-70 mt-1">
...
export default function ChatPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Generate and validate response in one call
      const generationResult = await guardly.generateAndValidate(
        inputValue,
        inputValue // In real app, this would be retrieved from context/docs
      );

      if (!generationResult) {
        throw new Error(guardly.error || 'Failed to generate response');
      }

      const assistantMessage: ChatMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: generationResult.generated_text,
        validation: {
          decision: generationResult.decision,
          risk_score: generationResult.risk_score,
          confidence: generationResult.confidence,
          evidence: generationResult.evidence,
          latency_ms: generationResult.latency.total_ms,
